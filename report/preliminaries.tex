...
The compiler used in this work is CompCert \addref{user manual}.
...
Instruction selection, Common sub-expression elimination, Tail call elimination, Dead code elimination, Function inlining, Branch tuneling, Constant propagation and Register allocation \addref{compcert overview}.
...
The logical model described here is similar to the one used in \addref{Kang et al}. However, it slightly differs from CompCert's current logical model described in \addref{memory model v2} (see section \addref{memupdate?}).
...
In \addref{Kang et al}, a new memory model is presented. The motivation is to have a memory model which allows integer-pointer casting, but still supports common optimizations.
...
The quasi-concrete is described in many details in \addref{Kang et al}.
