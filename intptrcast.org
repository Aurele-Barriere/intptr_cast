...
*** TODO rewrite extcall_properties
...
** TODO [INTPTR][mixedsim] strongly_receptive_at ?
   stronger or weaker? will it preserve the mixed -> backward proof?
   it adds the well_behaved_traces, but otherwise it seems wery similar because match traces only for [e]
** TODO [INTPTR][mixedsim] check if the suggested plan seems ok
...
** TODO [INTPTR][mixedsim] understand the changes when switching forward to mixed
...
** TODO [INTPTR][mixedsim] change mixed simulation
   Maybe remove receptivity completely.
   Then add it as an hypothesis when doing the mixed2backward proof.
   The mixed2backward proof won't be used anymore, maybe it can just be modified into the next theorem.
   When we add receptivity, we only need it for the "forward" states: all states but extcalls.
** TODO [INTPTR][mixedsim] new proof for mixed simulation
   maybe mixed A B /\ strongly_receptive A -> backward (atomic A) B
   Why will it be easier to prove for (atomic A) and not A?
...
** Contribution
*** memory update
    adding the consistency, new fields
    validity boolean removed, replaced with permission system
    -> what does it change?
    freeing concrete blocks...
*** memory injection
