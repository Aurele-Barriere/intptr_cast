...
** TODO [INTPTR][mixedsim] new proof for mixed simulation
   ...
   in that case there is an issue.
   or maybe if there are multiple events then we can do bacward reasonning for a while?
   or maybe we should direclty prove the bacward simulation, and have only one type of mixed_simulation.
