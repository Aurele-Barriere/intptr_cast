...
** TODO [INTPTR][sem] Determinism of capture
   This should remain non-deterministic.
   Then we will not have all the deterministic theorems.
   -> Mixed Bisimulations.

** TODO [INTPTR][sem] Change no op
   Define it with predicate?
   Use Mem.capture

** TODO [INTPTR][sem] implement extcall_capture_sem
*** TODO [INTPTR][sem] what should be the return value of a capture?
