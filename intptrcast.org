...
**** TODO [INTPTR][meminj][proofs] ValueDomain.v
     ...
     Since I've added the memory injection constraint saying that there can be no concrete source private block,
     there is one failing proof: mmatch_inj.
     This is located in backend/ValueDomain.v, where they try to prove a memory injection between a memory and itself,
     using the function inj_of_bc, which maps the block classified 'BCinvalid' to None.

     But I have no guarantee that BCinvalid blocks are logical, so I cannot prove the theorem.

     The reading of ValueDomain.v and ValueAnalysis.v lets me believe that BCinvalid blocks are
     - either previous stacks that get hidden when doing a privat function call
     - either blocks that haven't been defined yet.
       (even though, I don't have a theorem to say it)

     In the second case, these blocks will be logical. In the first case, I don't know.

     The mmatch_inj theorem is used only in external_call_match.
     This is because we need the injection to use the theorem external_call_mem_inject in Events.v

     Changing inj_of_bc might seem like a solution, but it would make the theorem inj_of_bc_inv false 
...
***** TODO [INTPTR][meminj][proofs][abstract] Add abstract concrete in amem
      with an abstract boolean?
      only one, for the stack frame
***** TODO [INTPTR][meminj][proofs][abstract] Change analyze_call
...
***** TODO [INTPTR][meminj][proofs][abstract] Show preservation of invalid_logical
      Two fails: external calls, and soundness of private calls
**** DONE [INTPTR][meminj][proofs] Unusedglobproof.v
