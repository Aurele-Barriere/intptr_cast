...
** TODO [INTPTR][mixedsim] understand the order in the factor proof
** TODO [INTPTR][mixedsim] strongly_receptive_at ?
   stronger or weaker? will it preserve the mixed -> backward proof?
** TODO [INTPTR][mixedsim] prove mixed A B -> mixed (atomic A) B ?
   i'm not sure if this is ok.
   i think that with the right definition of msim_forward it should be
** TODO [INTPTR][mixedsim] check if the suggested plan seems ok
** TODO [INTPTR][mixedsim] understand the proof with atomic
   where strongly receptive is used
   S forward T implies atomic(S) backward T
...
** TODO [INTPTR][mixedsim] new proof for mixed simulation
   where strongly receptive is used
   S mixed T implies atomic(S) backward T

   we had the proof fwd + strongly -> atomic bckwd
   we want the proof mixed + strongly -> atomic bckwd
