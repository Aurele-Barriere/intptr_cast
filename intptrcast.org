...
** TODO [INTPTR][memupdate] change memory extension
   ...
   I think the definition should not change when we add the capture
   It will only change because the definition of mem_inj might change
   However, how does the capture preserves memory extension?

   We'll see that after we agree on what changes to do to mem_inj, I guess.
