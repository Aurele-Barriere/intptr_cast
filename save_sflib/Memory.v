...
(* To implement the quasi-concrete memory-model, we introduce a new
   operation:
- [capture]: transform a logical block into a concrete one
 *)
...
(** In the quasi-concrete memory model, each block is associated to an option. If it is None, then the block is logical, else if it Some p, the block is concrete and its starts at address p. This is implemented with the field mem_concrete 

p should be given when performing the capture (or realization) of a block because of a int-ptr cast.

We also remember the low and high range of offsets that is available for a block in the field mem_offset_bounds.
...
Record mem' : Type := mkmem {
  ...
  mem_concrete: PMap.t (option Z); (** [block -> Z] **)
...
Definition memory_size :=
  if Archi.ptr64 then Int64.modulus else Int.modulus.


Lemma mkmem_ext:
 forall cont1 cont2 acc1 acc2 concrete1 concrete2 bounds1 bounds2 next1 next2 a1 a2 b1 b2 c1 c2,
  cont1=cont2 -> acc1=acc2 -> concrete1=concrete2 -> bounds1=bounds2 -> next1=next2 ->
  mkmem cont1 acc1 concrete1 bounds1 next1 a1 b1 c1 = mkmem cont2 acc2 concrete2 bounds2 next2 a2 b2 c2.
Proof.
...
Definition low_bound (m:mem) (b:block): Z :=
  let (lo,_) := m.(mem_offset_bounds)#b in lo.

Definition high_bound (m:mem) (b:block): Z :=
  let (_,hi) := m.(mem_offset_bounds)#b in hi.

Definition addr_in_concrete_block (m:mem) (addr:Z) (b:block) : Prop :=
  exists p,
  m.(mem_concrete)#b = Some p /\
  Z.le (p+ low_bound m b) addr /\
  Z.lt addr (p+ high_bound m b) /\
  perm m b (addr-p) Cur Nonempty.

Definition exists_at_most_one_block (P:block->Prop) : Prop:=
  forall block
         (ONEBLOCK: P block),
    <<ONLYONE: (forall b, b<>block -> ~P b)>>.
    
Definition no_concrete_overlap (m:mem) : Prop :=
  forall addr,
  exists_at_most_one_block (addr_in_concrete_block m addr).

Definition logical_nextblocks (m:mem) : Prop :=
  forall b
         (NEXTBLOCK: Ple m.(nextblock) b), 
    <<LOGICAL:m.(mem_concrete)#b = None>>.

Definition addresses_in_range (m:mem) : Prop :=
  forall addr b
         (INBLOCK: addr_in_concrete_block m addr b),
    <<INMEMORY: addr > 0 /\ addr < memory_size>>.


Definition mem_consistency (m:mem) : Prop :=
  no_concrete_overlap m /\
  logical_nextblocks m  /\
  addresses_in_range m.

(* [weaker_access m1 m2] states that the permissions of m1 are weaker than those of m2 *)
Definition weaker_access (m1:mem) (m2:mem): Prop :=
  forall b off permission
         (M1PERM: perm m1 b off Cur permission),
    <<M2PERM:perm m2 b off Cur permission>>.

Lemma weaker_access_no_overlap m1 m2
    (WEAK: weaker_access m1 m2)
    (NO_OVERLAP_M2: no_concrete_overlap m2)
    (CONCRETE_SAME: m1.(mem_concrete) = m2.(mem_concrete))
    (BOUNDS_SAME: m1.(mem_offset_bounds) = m2.(mem_offset_bounds)):
    no_concrete_overlap m1.
Proof.
  unfold no_concrete_overlap in *. unfold exists_at_most_one_block in *.
  unfold addr_in_concrete_block in *.
  intros. intro b. intro BNEQBLOCK. rewrite CONCRETE_SAME in *.
  unfold low_bound, high_bound in *.
  rewrite BOUNDS_SAME in *.
  destruct ONEBLOCK as [p [H1 [H2 [H3 H4]]]].
  apply WEAK in H4. 
  specialize NO_OVERLAP_M2 with (addr:=addr) (block0:=block).
  assert (H:(exists p : Z, (mem_concrete m2) # block = Some p /\ p + (let (lo, _) := (mem_offset_bounds m2) # block in lo) <= addr /\ addr < p + (let (_, hi) := (mem_offset_bounds m2) # block in hi) /\ perm m2 block  (addr - p) Cur Nonempty)).
  { exists p. split. exact H1. split.  exact H2. split. exact H3. exact H4. }
  clear H1 H2 H3 H4.
  apply NO_OVERLAP_M2 in H. clear NO_OVERLAP_M2.
  unfold not in *. intro H0. unfold NW in H. apply H with (b:=b). exact BNEQBLOCK.
  destruct H0 as [p1 H1]. destruct H1 as [H1 [H2 [H3 H4]]].
  exists p1. split. exact H1. split. exact H2. split. exact H3.
  apply WEAK. exact H4. 
Qed.
...
Program Definition empty: mem :=
  mkmem (PMap.init (ZMap.init Undef))
        ...
        (PMap.init None)        (* initially, blocks are logical *)
        (PMap.init (0,0))
        1%positive _ _ _.
Next Obligation.
...
Lemma no_concrete_overlap_empty:
  no_concrete_overlap empty.
Proof. 
  unfold no_concrete_overlap. unfold exists_at_most_one_block.
  intros. inversion ONEBLOCK as [x [H1 H2]]. inversion H1 as [H0].
  rewrite PMap.gi in H0. inversion H0.
Qed.

Lemma logical_nextblocks_empty:
  logical_nextblocks empty.
Proof.
  unfold logical_nextblocks. intros. apply PMap.gi.
Qed.

Lemma addresses_in_range_empty:
  addresses_in_range empty.
Proof. 
  unfold addresses_in_range. unfold addr_in_concrete_block.
  intros. simpl in INBLOCK. rewrite PMap.gi in INBLOCK.
  destruct INBLOCK as [x [H1 H2]]. inversion H1.
Qed.

Theorem mem_consistency_empty:
  mem_consistency empty.
Proof. 
  unfold mem_consistency. split.
  exact no_concrete_overlap_empty. split.
  exact logical_nextblocks_empty.
  exact addresses_in_range_empty.
Qed.
...
(** Allocation of a fresh block with the given bounds.  Return an updated
  ...
  infinite memory. *)
(* Here the "address" is a logical address: a block identifier without offset *)
...
Program Definition alloc (m: mem) (lo hi: Z) :=
  (mkmem (PMap.set m.(nextblock)
         ...
         (PMap.set m.(nextblock)
                   ...
                   m.(mem_access))
         (m.(mem_concrete))
         (PMap.set m.(nextblock) (lo,hi) m.(mem_offset_bounds))
         (Psucc m.(nextblock))
         _ _ _,
   m.(nextblock)).
Next Obligation.
...
| capture_ blk
           (SAMECONTENTS: m1.(mem_contents)   = m2.(mem_contents))
           (SAMEACCESS:   m1.(mem_access)     = m2.(mem_access))
           (SAMEOFFSET:   m1.(mem_offset_bounds) = m2.(mem_offset_bounds))
           (CONSISTENCY:  Mem.mem_consistency m2)
           (SAMECONCRTE:  (blk<>b -> m1.(mem_concrete)#blk = m2.(mem_concrete)#blk))
           (CAPTURED:     m2.(mem_concrete)#b = Some addr):
    <<CAPTURE: capture m1 b addr m2>>.

(** Casting between integers and pointers
- [valid_ptr] checks if a pointer is valid regarding to a memory
- [normalize] transforms a logical pointer into a concrete address
- [denormalize] find the logical pointer corresponding to a given concrete address
*)

(** A pointer (b,i) is valid if the block b is valid and i is smaller than the size of the block  *)
Definition valid_ptr (b:block) (off:Z) (m:mem): Prop := 
  let (lo,hi) := m.(mem_offset_bounds)#b in
  (Z.le lo off /\ Z.lt off hi) /\
  perm m b off Cur Readable.

(** Normalize transforms a block and offset into a concrete address if the block has been captured *)
Definition normalize (b:block) (off:Z) (m:mem): option Z :=
  match m.(mem_concrete)#b with
  | None => None                (* the block is still logical and hasn't been captured *)
  | Some p => Some (p + off)      (* should we check if off does not exeeds the range? *)
  end.

(** denormalize finds the block and offset corresponding to a concrete address *)
Inductive denormalize (addr:Z) (m:mem) (b:block) (i:Z): Prop :=
  |denormalize_ (p:Z)
                (BCONCRETE: m.(mem_concrete)#b = Some p)
                (IOFFSET:   p+i = addr)
                (INBLOCK:   addr_in_concrete_block m addr b):
     <<DENORMALIZE: denormalize addr m b i>>.
...
Proof.
  intros. exploit load_result; eauto; intros. rewrite x0.
...
Proof.
  ...
  unfold Ptrofs.max_unsigned. intro. omega.
Qed.
...
Theorem concrete_store_same:
  m1.(mem_concrete) = m2.(mem_concrete).
Proof.
  unfold store in STORE.
  destruct (valid_access_dec m1 chunk b ofs Writable); inversion STORE; try reflexivity.
Qed.  

Theorem offset_bounds_store_same:
  m1.(mem_offset_bounds) = m2.(mem_offset_bounds).
Proof.
  unfold store in STORE.
  destruct (valid_access_dec m1 chunk b ofs Writable); inversion STORE; try reflexivity.
Qed.

Theorem access_store_same:
  m1.(mem_access) = m2.(mem_access).
Proof.
  unfold store in STORE.
  destruct (valid_access_dec m1 chunk b ofs Writable); inversion STORE; try reflexivity.
Qed.
  
Theorem weaker_access_store:
  weaker_access m2 m1.
Proof.
  unfold weaker_access. unfold perm. rewrite access_store_same. auto.
Qed.
  
Theorem no_concrete_overlap_store
        (M1_NO_OVERLAP: Mem.no_concrete_overlap m1):
  <<M2_NO_OVERLAP: Mem.no_concrete_overlap m2>>.
Proof.

  apply weaker_access_no_overlap with (m2:=m1).
  apply weaker_access_store. exact M1_NO_OVERLAP.
  rewrite concrete_store_same. reflexivity.
  rewrite  offset_bounds_store_same. reflexivity.
Qed.

Theorem logical_nextblocks_store
        (M1_LOGICALNEXT: Mem.logical_nextblocks m1):
  <<M2_LOGICALNEXT: Mem.logical_nextblocks m2>>.
Proof.
  unfold logical_nextblocks. intro b0. intro HPLE.
  rewrite <- concrete_store_same. apply M1_LOGICALNEXT.
  rewrite <- nextblock_store. exact HPLE.
Qed.

Theorem addresses_in_range_store
        (M1_INRANGE:Mem.addresses_in_range m1):
  <<M2_INRANGE: Mem.addresses_in_range m2>>.
Proof. 
  unfold addresses_in_range in *. unfold addr_in_concrete_block in *.
  unfold low_bound in *. unfold high_bound in *. unfold perm in *.
  intro addr. intro b0. rewrite concrete_store_same in M1_INRANGE.
  rewrite <- offset_bounds_store_same.
  rewrite <- access_store_same.
  apply M1_INRANGE with (b:=b0) (addr:=addr).
Qed.

Theorem mem_consistency_store
        (M1_CONSISTENT: Mem.mem_consistency m1):
  <<M2_CONSISTENT:Mem.mem_consistency m2>>.
Proof. 
  unfold mem_consistency in *. destruct M1_CONSISTENT as [NOOVERLAP [LOGICAL INRANGE]]. 
  split. apply no_concrete_overlap_store. exact NOOVERLAP.
  split. apply logical_nextblocks_store. exact LOGICAL.
  apply addresses_in_range_store. exact INRANGE.
Qed. 


(* This is were I stopped rewriting proofs using Sflib **)
...
Theorem concrete_storebytes_same:
  m1.(mem_concrete) = m2.(mem_concrete).
Proof.
  unfold storebytes in STORE.
  destruct (range_perm_dec m1 b ofs (ofs + Z.of_nat (length bytes)) Cur Writable); inversion STORE; try reflexivity.
Qed.  

Theorem offset_bounds_storebytes_same:
  m1.(mem_offset_bounds) = m2.(mem_offset_bounds).
Proof.
  unfold storebytes in STORE.
  destruct (range_perm_dec m1 b ofs (ofs + Z.of_nat (length bytes)) Cur Writable); inversion STORE; try reflexivity.
...
Theorem access_storebytes_same:
  m1.(mem_access) = m2.(mem_access).
Proof.
  unfold storebytes in STORE.
  destruct (range_perm_dec m1 b ofs (ofs + Z.of_nat (length bytes)) Cur Writable); inversion STORE; try reflexivity.
...
Theorem weaker_access_storebytes:
  weaker_access m2 m1.
Proof.
  unfold weaker_access. unfold perm. rewrite access_storebytes_same.
  auto.
Qed.
  
Theorem no_concrete_overlap_storebytes:
    Mem.no_concrete_overlap m1 ->
    Mem.no_concrete_overlap m2.
Proof.
  intros.
  apply weaker_access_no_overlap with (m2:=m1).
  apply weaker_access_storebytes. exact H.
  rewrite concrete_storebytes_same. reflexivity.
  rewrite offset_bounds_storebytes_same. reflexivity.
Qed.

Theorem logical_nextblocks_storebytes:
  Mem.logical_nextblocks m1 ->
  Mem.logical_nextblocks m2.
Proof.
  unfold logical_nextblocks. intros.
  rewrite <- concrete_storebytes_same. apply H.
  rewrite <- nextblock_storebytes. exact NEXTBLOCK.
Qed.

Theorem addresses_in_range_storebytes:
  Mem.addresses_in_range m1 ->
  Mem.addresses_in_range m2.
Proof. 
  unfold addresses_in_range. unfold addr_in_concrete_block.
  unfold low_bound. unfold high_bound. unfold perm.
  intros. rewrite concrete_storebytes_same in H.
  apply H with (b:=b0).
  rewrite offset_bounds_storebytes_same.
  rewrite access_storebytes_same.
  exact INBLOCK.
Qed.

Theorem mem_consistency_storebytes:
  Mem.mem_consistency m1 -> Mem.mem_consistency m2.
Proof. 
  unfold mem_consistency. intros. destruct H as [H1 H]. destruct H as [H2 H3].
  split. apply no_concrete_overlap_storebytes. exact H1.
  split. apply logical_nextblocks_storebytes. exact H2.
  apply addresses_in_range_storebytes. exact H3.
Qed. 
...
Proof.
  ...
  rewrite PMap.gso. auto. rewrite H0. apply sym_not_equal; eauto with mem.
  rewrite pred_dec_false. auto.
  eauto with mem.
Qed.
...
Theorem load_alloc_same:
  ...
  v = Vundef.
Proof.
  intros. exploit load_result; eauto. intro. rewrite x0.
  injection ALLOC; intros. rewrite <- H1; simpl. rewrite <- H0.
...
Theorem concrete_alloc_same:
  m1.(mem_concrete) = m2.(mem_concrete).
Proof.
  unfold alloc in ALLOC. inversion ALLOC. reflexivity.
Qed.  

Theorem offset_bounds_alloc_same:
  forall blk,
    blk <> m1.(nextblock) -> m1.(mem_offset_bounds)#blk = m2.(mem_offset_bounds)#blk.
Proof.
  unfold alloc in ALLOC. inversion ALLOC.
  intros. apply PMap.gso with (x:=(lo,hi)) (m:=m1.(mem_offset_bounds)) in H.
  rewrite <- H. reflexivity.
Qed.    

Theorem access_alloc_same:
  forall blk,
    blk <> m1.(nextblock) -> m1.(mem_access)#blk = m2.(mem_access)#blk.
Proof.
  unfold alloc in ALLOC. inversion ALLOC.
  intros. apply PMap.gso with (x:= (fun (ofs : Z) (_ : perm_kind) => if zle lo ofs && zlt ofs hi then Some Freeable else None))(m:=m1.(mem_access)) in H.
  rewrite <- H. reflexivity.
Qed.    

Theorem no_concrete_overlap_alloc
    (LOGICAL_NEXT: Mem.logical_nextblocks m1)
    (NO_OVERLAP_M1: Mem.no_concrete_overlap m1):
    Mem.no_concrete_overlap m2.
Proof. 
  unfold no_concrete_overlap in *. unfold exists_at_most_one_block in *.
  unfold addr_in_concrete_block in *. unfold not in *.
  intros. intro b0. intro HBNEQBLOCK.
  rewrite concrete_alloc_same in *.
  destruct (Pos.eq_dec b0 m1.(nextblock)).
  - (* b0 = nextblock m1 = b *)
    unfold alloc in ALLOC.
    assert(HCONCRETE: m1.(mem_concrete)#(m1.(nextblock))=None).
    { apply LOGICAL_NEXT. apply Ple_refl. }
    intro HFALSE.
    rewrite <- concrete_alloc_same in HFALSE.
    rewrite e in HFALSE.
    destruct HFALSE as [p [HFALSE H2]]. rewrite HCONCRETE in HFALSE. 
    inversion HFALSE.    
  - (* b0 != nextblock m1 *)
    intro HFALSE.
    apply NO_OVERLAP_M1 with (addr:=addr) (b:=block) (block0:=b0).
    + destruct HFALSE as [p [H1[H2 [H3 H4]]]]. exists p.
    split. exact H1.
    assert(BOUNDS_B0:(mem_offset_bounds m1) # b0 =
                     (mem_offset_bounds m2) # b0).
    { apply offset_bounds_alloc_same. apply n. }
    split. unfold low_bound in *. rewrite BOUNDS_B0 in *. exact H2.
    split. unfold high_bound in *. rewrite BOUNDS_B0 in *. exact H3.
    assert(ACCESS_B0:(mem_access m1) # b0 = (mem_access m2) # b0).
    { apply access_alloc_same. apply n. }
    unfold perm. rewrite ACCESS_B0. exact H4.
    + intros. apply HBNEQBLOCK. rewrite H. reflexivity.
    + destruct (Pos.eq_dec block m1.(nextblock)).
      * destruct ONEBLOCK as [p [H H2]]. rewrite e in H.
        rewrite <- concrete_alloc_same in H.
        assert(HNONE: m1.(mem_concrete)#(m1.(nextblock))=None).
        { apply LOGICAL_NEXT. apply Ple_refl. }
        unfold not in n. Admitted. (*
      * clear H1 NO_OVERLAP_M1.
        destruct H as [p [H1 [H2 [H3 H4]]]].
        assert(BOUNDS_BLOCK:(mem_offset_bounds m1) # block =
                            (mem_offset_bounds m2) # block).
        { apply offset_bounds_alloc_same. apply n0. }
        exists p. split. exact H1. split.
        unfold low_bound in *. rewrite BOUNDS_BLOCK in *. exact H2. split.
        unfold high_bound in *. rewrite BOUNDS_BLOCK in *. exact H3.
        assert(ACCESS_BLOCK:(mem_access m1) # block = (mem_access m2) # block).
        { apply access_alloc_same. apply n0. }
        unfold perm. rewrite ACCESS_BLOCK. exact H4.      
Qed. *)

Theorem logical_nextblocks_alloc:
  Mem.logical_nextblocks m1 ->
  Mem.logical_nextblocks m2.
Proof.
  unfold logical_nextblocks. intros.
  rewrite <- concrete_alloc_same. apply H.
  rewrite nextblock_alloc in H0.
  apply Ple_trans with (q:=Pos.succ(nextblock m1)).
  apply Ple_succ. exact H0.
Qed.

Theorem addresses_in_range_alloc:
  forall
    (LOGICAL_NEXT: Mem.logical_nextblocks m1)
    (IN_RANGE_M1: Mem.addresses_in_range m1),
    Mem.addresses_in_range m2.
Proof. 
  unfold addresses_in_range. unfold addr_in_concrete_block.
  unfold low_bound. unfold high_bound.
  intros. rewrite concrete_alloc_same in IN_RANGE_M1.
  apply IN_RANGE_M1 with (b:=b0).
  destruct H as [p [H1 [H2 [H3 H4]]]].
  destruct (Pos.eq_dec b0 m1.(nextblock)).
  - rewrite e in H1. rewrite <- concrete_alloc_same in H1.
    assert ((mem_concrete m1) # (nextblock m1) = None).
    { apply LOGICAL_NEXT. apply Ple_refl. }
    rewrite H in H1. inversion H1.
  - rewrite offset_bounds_alloc_same.
  exists p.
  split. apply H1.
  split. apply H2.
  split. apply H3.
  unfold perm. apply access_alloc_same in n. rewrite n. exact H4.
  exact n.  
Qed.


Theorem mem_consistency_alloc:
  Mem.mem_consistency m1 -> Mem.mem_consistency m2.
Proof. 
  unfold mem_consistency. intros. destruct H as [H1 H]. destruct H as [H2 H3].
  split. apply no_concrete_overlap_alloc. exact H2. exact H1.
  split. apply logical_nextblocks_alloc. exact H2.
  apply addresses_in_range_alloc. exact H2. exact H3.  
Qed.
...
Theorem concrete_free_same:
  m1.(mem_concrete) = m2.(mem_concrete).
Proof.
  unfold free in FREE.
  destruct (range_perm_dec m1 bf lo hi Cur Freeable); inversion FREE; try reflexivity.
Qed.

Theorem offset_bounds_free_same:
  m1.(mem_offset_bounds) = m2.(mem_offset_bounds).
Proof.
  unfold free in FREE.
  destruct (range_perm_dec m1 bf lo hi Cur Freeable); inversion FREE; try reflexivity.
...
Theorem weaker_access_free:
  weaker_access m2 m1.
Proof.
  unfold free in FREE; inversion FREE; clear FREE.
  destruct (range_perm_dec m1 bf lo hi Cur Freeable) in H0; inversion H0. clear H0.
  unfold weaker_access.
  intros.
  unfold unchecked_free in H.
  unfold perm in *. simpl in H.
  destruct (Pos.eq_dec b bf).
  - rewrite e in *. clear e.
    rewrite PMap.gss in H.
    destruct (zle lo off && zlt off hi) eqn:HRANGE; try inversion H; try exact H.
  - rewrite PMap.gso in H; try exact n.
    exact H.
Qed.
    
Theorem no_concrete_overlap_free:
    Mem.no_concrete_overlap m1 ->
    Mem.no_concrete_overlap m2.
Proof.
  intros.
  apply weaker_access_no_overlap with (m2:=m1).
  apply weaker_access_free. exact H.
  rewrite concrete_free_same. reflexivity.
  rewrite offset_bounds_free_same. reflexivity.
Qed.
  
Theorem logical_nextblocks_free:
  Mem.logical_nextblocks m1 ->
  Mem.logical_nextblocks m2.
Proof.
  unfold logical_nextblocks. intros.
  rewrite <- concrete_free_same. apply H.
  rewrite <- nextblock_free. exact H0.
Qed.

Theorem addresses_in_range_free:
  Mem.addresses_in_range m1 ->
  Mem.addresses_in_range m2.
Proof. 
  unfold addresses_in_range. unfold addr_in_concrete_block.
  unfold low_bound. unfold high_bound. unfold perm.
  intros. rewrite concrete_free_same in H.
  apply H with (b:=b).
  rewrite offset_bounds_free_same.
  destruct H0 as [p H0].
  exists p. destruct H0 as [H1 H2]. destruct H2 as [H2 H3]. destruct H3 as [H3 H4].
  split. apply H1.
  split. apply H2.
  split. apply H3.
  apply weaker_access_free.
  apply H4.
Qed.


Theorem mem_consistency_free:
  Mem.mem_consistency m1 -> Mem.mem_consistency m2.
Proof.  
  unfold mem_consistency. intros. destruct H as [H1 H]. destruct H as [H2 H3].
  split. apply no_concrete_overlap_free. exact H1.
  split. apply logical_nextblocks_free. exact H2.
  apply addresses_in_range_free. exact H3.
Qed.
...
Theorem concrete_drop_perm_same:
  m.(mem_concrete) = m'.(mem_concrete).
Proof.
  unfold drop_perm in DROP.
  destruct (range_perm_dec m b lo hi Cur Freeable); inversion DROP; try reflexivity.
Qed.

Theorem offset_bounds_drop_perm_same:
  m.(mem_offset_bounds) = m'.(mem_offset_bounds).
Proof.
  unfold drop_perm in DROP.
  destruct (range_perm_dec m b lo hi Cur Freeable); inversion DROP; try reflexivity.
Qed.

Theorem weaker_access_drop_perm:
  weaker_access m' m.
Proof.
  unfold drop_perm in DROP.
  destruct (range_perm_dec m b lo hi Cur Freeable) in DROP; inversion DROP. clear DROP.
  unfold weaker_access.
  intros. clear H0.
  unfold perm in *. simpl in H.
  ...
  - rewrite e in *. clear e.
    rewrite PMap.gss in H.
    destruct (zle lo off && zlt off hi) eqn:HRANGE.
    + unfold range_perm in r.
      assert (perm m b off Cur Freeable).
      { apply r. apply andb_true_iff in HRANGE.
        destruct HRANGE as [HLO HHI]. split.
        destruct zle in HLO. exact l. inversion HLO.
        destruct zlt in HHI. exact l. inversion HHI. }
      unfold perm in H0. unfold perm_order' in *.
      destruct ((mem_access m) # b off Cur);
      try induction p0; inversion H0; apply perm_F_any.
    + exact H.
  - rewrite PMap.gso in H; try exact n. exact H. 
Qed.

    
Theorem no_concrete_overlap_drop_perm:
    Mem.no_concrete_overlap m ->
    Mem.no_concrete_overlap m'.
Proof.
  intros.
  apply weaker_access_no_overlap with (m2:=m).
  apply weaker_access_drop_perm. exact H.
  rewrite concrete_drop_perm_same. reflexivity.
  rewrite offset_bounds_drop_perm_same. reflexivity.
Qed.

Theorem logical_nextblocks_drop_perm:
  Mem.logical_nextblocks m ->
  Mem.logical_nextblocks m'.
Proof.
  unfold logical_nextblocks. intros.
  rewrite <- concrete_drop_perm_same. apply H.
  rewrite <- nextblock_drop. exact H0.
Qed.

Theorem addresses_in_range_drop_perm:
  Mem.addresses_in_range m ->
  Mem.addresses_in_range m'.
Proof. 
  unfold addresses_in_range. unfold addr_in_concrete_block.
  unfold low_bound. unfold high_bound.
  intros. rewrite concrete_drop_perm_same in H.
  apply H with (b:=b0).
  rewrite offset_bounds_drop_perm_same.
  destruct H0 as [p0 H0]. destruct H0 as [H1 H2]. destruct H2 as [H2 H3]. destruct H3 as [H3 H4].
  exists p0.
  split. apply H1.
  split. apply H2.
  split. apply H3.
  apply weaker_access_drop_perm. apply H4.
Qed.

Theorem mem_consistency_drop_perm:
  Mem.mem_consistency m -> Mem.mem_consistency m'.
Proof. 
  unfold mem_consistency. intros. destruct H as [H1 H]. destruct H as [H2 H3].
  split. apply no_concrete_overlap_drop_perm. exact H1.
  split. apply logical_nextblocks_drop_perm. exact H2.
  apply addresses_in_range_drop_perm. exact H3.
Qed.
...
Theorem mem_consistency_capture:
    Mem.mem_consistency m1 ->
    Mem.mem_consistency m2.
Proof.
  intros. inversion CAPTURE. exact H4.
Qed.
